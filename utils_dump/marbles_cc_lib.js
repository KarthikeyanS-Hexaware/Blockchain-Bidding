...
module.exports = function (enrollObj, g_options, fcw, logger) {
	...
	marbles_chaincode.fileView = function (options, cb) {
		...
		var opts = {
			...
			cc_args: [options.args.shipment_id, options.args.file_name]
		};
		fcw.query_chaincode(enrollObj, opts, cb);
	};
	...
	marbles_chaincode.getAllLCs = function (options, cb) {
		logger.info('Getting all LCs', options.args);
